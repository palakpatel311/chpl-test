...
public class MonitorDownloadFilesSteps {
    ...
    /**
     * Get user to the Download CHPL page.
     * Chrome options are necessary to get past keep/discard pop ups for successful download of a file to directory.
     */  
    ...
    public void the_downloaded_file_shows_chpl_2015_currentdate_xml_filename() {
         ...
         * and sort it by last modified date to get latest download file in the directory.
         * This can be improved by deleting each file in the directory after validation (file.delete() was not working).
         */
        File[] files = dir.listFiles();
        Arrays.sort(files, LastModifiedFileComparator.LASTMODIFIED_REVERSE);
        /**
         * Get latest download file and replace ddmmhh part to construct a filename to match with expected
         * Use of DateFormat to get current system date 
         */ 
        for (File file : files) {                          
            String matchFile = file.getName();
        String downloadfileName = matchFile.replaceAll("(^chpl-2015-........_).{2,10}(.xml)", "$1"+"xxxxxx"+"$2");        
    ...
    public void the_downloaded_file_shows_chpl_2014_currentdate_xml_filename() {
        ...
        for (File file : files) {                          
        String matchFile = file.getName();
        String downloadfileName = matchFile.replaceAll("(^chpl-2014-........_).{2,10}(.xml)", "$1"+"xxxxxx"+"$2");        
        String sysfilename = new SimpleDateFormat("yyyyMMdd_").format(new Date());
  ...
  @Then("^the downloaded file shows 2011-chpl-lastrun.xml filename$")
    public void the_downloaded_file_shows_chpl_2011_lastrun_xml_filename() {
          ...
          String matchFile = file.getName();
           ...
           String downloadfileName = matchFile.replaceAll("(^chpl-2011-........_).{2,10}(.xml)", "$1"+"xxxxxx"+"$2"); 
           
           String sysMonth = new SimpleDateFormat("MM").format(new Date());
           String sysYear = new SimpleDateFormat("yyyy").format(new Date()); 
           /**
            * Construct filename for each quarter to validate download filename. 
            * 2011 product xml file is generated on first day of a quarter.
            * Compare filenames generated within a quarter against constructed filename.
            */
           ...
           if (sysMonth.equalsIgnoreCase("01") || sysMonth.equalsIgnoreCase("02") || sysMonth.equalsIgnoreCase("03") && downloadfileName.equals(sysfilenameQ1)) {
               assertEquals(downloadfileName,sysfilenameQ1, "File is not current");}
           else if (sysMonth.equalsIgnoreCase("04") || sysMonth.equalsIgnoreCase("05") || sysMonth.equalsIgnoreCase("06") && downloadfileName.equals(sysfilenameQ2)) {
               assertEquals(downloadfileName,sysfilenameQ2, "File is not current");}
           
           else if (sysMonth.equalsIgnoreCase("07") || sysMonth.equalsIgnoreCase("08") || sysMonth.equalsIgnoreCase("09") && downloadfileName.equals(sysfilenameQ3)) {
               assertEquals(downloadfileName,sysfilenameQ3, "File is not current");}
           else if (sysMonth.equalsIgnoreCase("10") || sysMonth.equalsIgnoreCase("11") || sysMonth.equalsIgnoreCase("12") && downloadfileName.equals(sysfilenameQ4)) {
    ...
    public void the_downloaded_file_shows_chpl_2015_currentdate_csv_filename() {
        ...
        for (File file : files) {                          
        String matchFile = file.getName();
        String downloadfileName = matchFile.replaceAll("(^chpl-2015-........_).{2,10}(.csv)", "$1"+"xxxxxx"+"$2");        
        String sysfilename = new SimpleDateFormat("yyyyMMdd_").format(new Date());
    ...
    public void the_downloaded_file_shows_chpl_2014_currentdate_csv_filename() {
        ...
        for (File file : files) {                          
        String matchFile = file.getName();
        String downloadfileName = matchFile.replaceAll("(^chpl-2014-........_).{2,10}(.csv)", "$1"+"xxxxxx"+"$2");                  
        String sysfilename = new SimpleDateFormat("yyyyMMdd_").format(new Date());
