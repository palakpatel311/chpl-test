...
Scenario Outline: 
    ...
    Then the "<Downloaded File Name>" ends with "yyyyMMdd_HHmmss" 
    Examples: 
        |File                 |Downloaded File Name             |
        |Surveillance Activity|surveillance-all                 |
        |Non-Conformities     |surveillance-with-nonconformities|

Scenario: 
    At any given time, when downloaded, SED Details file name ends with Date and time format 
    Given I am on "SED Information for 2015 Edition" collections page: "sed" 
    When I download the "chpl-sed" file 
    Then the "chpl-sed" ends with "yyyyMMdd_HHmmss"